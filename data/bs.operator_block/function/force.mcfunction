...
execute positioned over world_surface positioned ~ ~1 ~ summon area_effect_cloud store result storage bs.operator_block:choose_block coords.y int 1 run return run data get entity @s Pos[1]

# highest block in world that is garanteed to exist
execute positioned over world_surface summon area_effect_cloud store result storage bs.operator_block:choose_block coords.y int 1 run data get entity @s Pos[1]
